...
while True:
    ...
    for i in range(len(contours)):
        ...
        if area < 1500 and area > 15000: # is piece or base ?
            ...
            if abs(w - h) < 5: 
                m = np.array([[cos(r*pi/180),-sin(r*pi/180)],[sin(r*pi/180),cos(r*pi/180)]])
                a = np.array([0-43/2,0-43/2])
                b = np.array([0+43/2,0+43/2])
                c = np.array([0+43/2,0-43/2])
                d = np.array([0-43/2,0+43/2])
                a = m.dot(a.T) + np.array([cx, cy])
                b = m.dot(b.T) + np.array([cx, cy])
                c = m.dot(c.T) + np.array([cx, cy])
                d = m.dot(d.T) + np.array([cx, cy])
    ...
    for i in range(4):
        if my_pice_cornors[i,0] != 0 and my_pice_cornors[i,1] != 0 and base_pice_cornors[i,0]!=0 and base_pice_cornors[i,1]!=0:
            ...
            if i == 0: ang = my_pice_cornors[i,2] - base_pice_cornors[i, 2]
            if i == 1: ang = my_pice_cornors[i,2] - base_pice_cornors[i, 2] + 270
            if i == 2: ang = my_pice_cornors[i,2] - base_pice_cornors[i, 2] - 270
            if i == 3: ang = my_pice_cornors[i,2] - base_pice_cornors[i, 2] - 180
            ...
            pub_list[i*5] = my_pice_cornors[i,0]
            pub_list[i*5+1] = my_pice_cornors[i,1]
            pub_list[i*5+2] = base_pice_cornors[i,0]
            pub_list[i*5+3] = base_pice_cornors[i,1]
            pub_list[i*5+4] = ang
    pub_list_ = pub_list
    cv2.imshow('img', img)
